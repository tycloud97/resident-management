...
export default function ComplaintForm() {
  ...
  const severityKey = form.severity as keyof typeof SEVERITY_LABEL
  const severityLabel = SEVERITY_LABEL[severityKey]
  const slaMinutes = SEVERITY_SLA_MINUTES[severityKey]

  return (
    <div className="max-w-2xl">
      <h1 className="text-2xl font-bold mb-4">Gửi phản ánh</h1>
      <div className="mb-3 text-base text-gray-700">
        Bạn có thể gửi <b>ẩn danh</b> hoặc bỏ ẩn danh để điền thông tin liên hệ.
      </div>
      {serverError && (
        <div className="mb-3">
          <Alert type="error" title="Lỗi">
            {serverError}
          </Alert>
        </div>
      )}
      <form onSubmit={handleSubmit} className="space-y-4" noValidate>
        <div className="space-y-3 bg-white rounded-lg shadow-sm p-4">
          <h2 className="text-lg font-semibold text-gray-900">Phân loại &amp; mức độ xử lý</h2>
          <p className="text-sm text-gray-600">
            Mức độ bạn chọn sẽ ảnh hưởng đến mức độ ưu tiên xử lý. Với lựa chọn hiện tại{' '}
            <b>{severityLabel}</b>, Ban quản lý cố gắng phản ứng trong khoảng{' '}
            <b>{slaMinutes} phút</b>.
          </p>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            <Select label="Loại" value={form.type} onChange={(e) => set('type', e.target.value)} required>
              <option value="MAINTENANCE">Sửa chữa</option>
              <option value="NOISE">Tiếng ồn</option>
              <option value="SECURITY">An ninh</option>
              <option value="OTHER">Khác</option>
            </Select>
            <Select label="Mức độ nghiêm trọng" value={form.severity} onChange={(e) => set('severity', e.target.value)} required>
          ...
          <p className="text-xs text-gray-500">
            Thấp: 120 phút · Trung bình: 30 phút · Cao: 15 phút · Khẩn cấp: 5 phút
          </p>
        </div>

        <div className="space-y-3 bg-white rounded-lg shadow-sm p-4">
          <h2 className="text-lg font-semibold text-gray-900">Nội dung phản ánh</h2>
        ...
        <div className="space-y-3 bg-white rounded-lg shadow-sm p-4">
          <h2 className="text-lg font-semibold text-gray-900">Thông tin căn hộ</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            <Select
              label="Tòa"
              value={form.building}
        ...
        <div className="space-y-3 bg-white rounded-lg shadow-sm p-4">
          <h2 className="text-lg font-semibold text-gray-900">Ẩn danh & thông tin liên hệ</h2>
        ...
        <div className="space-y-3 bg-white rounded-lg shadow-sm p-4">
          <h2 className="text-lg font-semibold text-gray-900">Ảnh / video minh chứng</h2>
          <p className="text-sm text-gray-500">
            Bạn có thể tải lên tối đa 5 ảnh, dung lượng tối đa 5MB/ảnh để minh họa rõ hơn cho phản ánh của mình.
          </p>
          <EvidenceUploader onChange={setFiles} />
        </div>

        <div className="flex gap-2">
          <Button type="submit" disabled={mutation.isPending} className="py-3 px-5 text-base">
            {mutation.isPending ? 'Đang gửi…' : 'Gửi phản ánh'}
          </Button>
          <Button type="button" variant="secondary" onClick={() => history.back()} className="py-3 px-5 text-base">
            Hủy
          </Button>
        </div>
      </form>
